...
fun VideoList(
    ...
    if (horizontal) {
        Row(modifier) {
            for (rawIndex in 0..<videoIdList.size) {
                Box(modifier = Modifier.weight(1.0f)) {
                    videoIdList.getOrNull(rawIndex)?.let { id ->
                        VideoCell(
...
fun ActionButtonsPreview() {
    ActionButtons(text = "Press to join ...") { }
}

@Composable
fun ActionButtons(modifier: Modifier = Modifier, text: String, onClick: () -> Unit) {
    Row(modifier = modifier.fillMaxWidth()) {
        Button(modifier = modifier, content = { Text(text) }, onClick = onClick)
    }
}
...
fun VideoCellPreview() {
    VideoCell(id = 0, isLocal = true, isSelected = false, setupVideo = { _, _, _ ->
    })
}
